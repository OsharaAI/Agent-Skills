...
Concrete workflow YAML and scripts for PR coverage diffs, threshold configuration, and the ratchet pattern. The reasoning behind these choices lives in `SKILL.md`; this file is implementation only.

## Coverage Diff on PRs

Surface the coverage change on each PR so reviewers can see the impact at a glance.
...
Alternative approach: pair `marocchino/sticky-pull-request-comment@v2` with a custom script that reads `coverage-summary.json`, narrows the results to changed files (found via `git diff --name-only origin/main...HEAD`), and builds a markdown table of per-file line and branch numbers.

## Threshold Configuration

**Global threshold (the floor for the whole project):**
...
**Per-directory thresholds (tighter for critical code):**
...
Coverage should only move upward. Capture the current level as the floor, then raise that floor whenever coverage climbs higher.
...
Commit `.coverage-ratchet.json` to the repo (for example, `{ "lines": 82, "branches": 78, ... }`). Run the ratchet script in CI right after the test step. When a merge to the main branch raises coverage, auto-commit the updated ratchet file as part of that pipeline.
