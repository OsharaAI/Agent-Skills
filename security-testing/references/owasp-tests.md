...
Runnable Playwright implementations for each OWASP 2025 category. `SKILL.md` carries the "what to test" reasoning and the checklist; this file is where the actual test code lives.

## Asserting Against a Set of Acceptable Status Codes

A number of these tests have more than one legitimate outcome — an SSRF block, for instance, might reasonably come back as 400, 403, or 422 depending on where it's caught. **`toBeOneOf` is not a built-in Playwright/Jest/Vitest matcher.** Writing `expect(x).toBeOneOf([...])` will throw `toBeOneOf is not a function` the moment it runs. Use the built-in `toContain`, flipped around the acceptable-values array, instead:
...
If the `toBeOneOf` phrasing genuinely reads better to you, you can register it as a custom matcher once at suite setup (`expect.extend({ toBeOneOf(received, set) { return { pass: set.includes(received), message: () => \`expected \${received} in \${set}\` }; } });`). Everything below sticks to the matcher-free `toContain` form so it runs without any extra setup.

## A01: Broken Access Control

Covers IDOR, vertical privilege escalation, CORS misconfiguration, and SSRF — the latter now classified as an access-control failure whenever the server can be induced to reach internal resources on request.
...
Checks that production doesn't leak internals through error output and that legacy/debug HTTP methods stay disabled.
...
New in 2025 — covers provenance, build-pipeline integrity, and SBOM generation.
