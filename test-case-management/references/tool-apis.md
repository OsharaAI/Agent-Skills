...
These four tools resemble each other on the surface, but their APIs are genuinely distinct.
The single most common agent mistake is cross-wiring them: putting TestRail's
`custom_steps_separated` on a Qase body, sending Qase's `Token` header to Zephyr, using Bearer
auth on Qase, or hitting the deprecated `/rest/raven/1.0/` path on Xray Cloud. Always copy
syntax from the matching tool's section below.
...
The endpoint is `add_case/{section_id}` — a case attaches to a **section**, not directly to a
suite. Don't confuse `add_case` (a test case living in the repository) with `add_test` (a test
instance inside a run — a different thing entirely). Getting per-step separated steps requires
the `custom_steps_separated` custom field, an array of `{content, expected}` objects. The plain
`custom_steps` field is just a single text blob and won't give you per-step expected results.

Auth is HTTP Basic using `email:api_key` (or `email:password`) — not a bearer token.
...
For bulk creation, loop a single `add_case/{section_id}` call per case — TestRail has no native
batch create-case endpoint. Generate one case per acceptance rule rather than one giant case.

`template_id: 2` is usually the built-in "Test Case (Steps)" template, but confirm the actual
id via `get_templates/{project_id}` since it can vary by project.

---

## Xray on Jira Cloud — two-step auth, then GraphQL

Watch for stale knowledge here: the Cloud REST import path `/rest/raven/1.0/...` actually
belongs to **Server/DC** and is the wrong call on Cloud. For Xray Cloud, authenticate first,
then use the **GraphQL** API (`/api/v2/graphql`) — Xray's own docs recommend GraphQL for
integration work since it's more capable than the REST helpers.
...
Don't use Jira username/password basic auth for the Xray API — that authenticates against Jira
itself, not Xray. The Xray API key (client_id + client_secret) comes from Xray Global Settings.
...
POST this query to `https://xray.cloud.getxray.app/api/v2/graphql` with
`Authorization: Bearer $TOKEN`.

### Step 2b — import a Gherkin scenario as a **Cucumber** test (not Generic)

A frequent mistake is importing Gherkin as a `Generic` test, which just dumps it unparsed into
a definition field. Instead use `testType: { name: "Cucumber" }` with the `gherkin` field so
Xray actually parses the scenario:
...
For a batch of 12 cases, alias multiple `createTest` calls together in one GraphQL document, or
loop the calls. There's also a REST helper, `POST /api/v2/import/test/bulk`, for JSON-defined
manual tests, but GraphQL `createTest` remains the cleaner default for mixed manual + Cucumber
sets.
...
The correct base is `https://api.zephyrscale.smartbear.com/v2` with
`Authorization: Bearer <JWT>` (the API access token generated in Zephyr Scale settings). Common
mistakes here: emitting `/v1/`, using the wrong host, leaking in Qase's `Token` header, or
trying to cram steps as one text blob into the create-case body. Test steps require a
**separate call**: `POST /testcases/{caseKey}/teststeps`.
...
The response includes the new test case `key` (e.g. `WEB-T42`), which you'll need for step 2.
`folderId` is numeric — look up the "Smoke" folder via
`GET /folders?projectKey=WEB&folderType=TEST_CASE`.
...
Each step takes the form of an `inline` object with `description`, `expectedResult`, and an
optional `testData`. (`testCaseKey` can be used in place of `inline` when reusing a shared test
case as a step.)
...
The auth header here is `Token: <api_key>` — NOT `Authorization: Bearer`. Defaulting to Bearer
is the single most common Qase mistake. Use the bulk endpoint `POST /case/{PROJECT_CODE}/bulk`
with a `cases` array rather than issuing N separate `POST /case/{CODE}` calls. Steps are
structured objects carrying `action`, `expected_result`, and `data` (input data).
...
`CHK` here is the project code (the short code visible in the project URL), and `suite_id` is
the numeric parent suite. Keep it to one case per business rule.
</content>
