...
Concrete test-idea lists covering boundary, state-transition, error-handling, and "what if" exploration, plus the exploration-to-automation pipeline and a worked Playwright regression example. The HICCUPS / FEW HICCUPS oracle tables and the explore-vs-automate decision framework are covered in `SKILL.md`.

## Boundary Testing Heuristics

Bugs cluster at boundaries. Work through these systematically:
...
Focus on how the application moves from one state to another:
...
Open-ended prompts that tend to surface bugs:
...
**Turning a finding into an automated test:**

1. Pull the exact reproduction steps straight from the session log
2. Pick the right test level (unit for a logic bug, integration for a boundary bug, E2E for a UI flow bug)
3. Write the test and reference the original bug ID clearly
4. Confirm it fails on the buggy build and passes once the fix lands
