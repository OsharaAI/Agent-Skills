...
This file holds the runnable code for the common API testing patterns. The reasoning behind them, the anti-patterns, and the "done when" checklist all live in `SKILL.md`.
...
Headers are part of the contract as much as the body: `content-type`, cache directives, rate-limit info. Check them directly rather than hiding the assertion behind an `if` that may never run.
...
Arguably the most valuable GraphQL test you can write: confirm that fields and resolvers haven't quietly vanished without a migration note. Snapshot the introspected type map and fail whenever it shrinks unexpectedly — losing a field is a breaking change for every downstream consumer.
