...
describe('user management', () => {
  it('A: should create a user', async () => {
    await api.post('/users', { name: 'Priya' });
  });
  it('B: should list users', async () => {
    const users = await api.get('/users');
    expect(users).toContainEqual({ name: 'Priya' }); // Fails if A didn't run first
  });
});
...
describe('user management', () => {
  it('should create a user', async () => {
    const response = await api.post('/users', { name: 'Priya' });
    expect(response.status).toBe(201);
  });
  it('should list users including recently created', async () => {
    await api.post('/users', { name: 'Marco' }); // Own setup
    const users = await api.get('/users');
    expect(users).toContainEqual({ name: 'Marco' });
  });
});
```
...
it('should handle user lifecycle', async () => {
  const user = await service.create({ name: 'Priya' });
  expect(user.id).toBeDefined();

  await service.update(user.id, { name: 'Marco' });
  const updated = await service.get(user.id);
  expect(updated.name).toBe('Marco');
...
it('should update user name', async () => {
  const user = await service.create({ name: 'Priya' });
  await service.update(user.id, { name: 'Marco' });
  expect((await service.get(user.id)).name).toBe('Marco');
});
