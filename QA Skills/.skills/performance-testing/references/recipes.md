...
This file holds the runnable code for the performance-testing skill. The reasoning,
principles, and Done-When checklist stay in `SKILL.md`; everything here is implementation.
...
A spike test isn't finished until "does it come back?" is a real assertion rather than a
stray comment. Label each phase of the run, then scope a threshold to the post-spike
`recovery` window so the job fails if p95 never settles back near baseline. Compare it
against the normal-load threshold to demonstrate the system genuinely bounced back.
...
Want something lighter-weight? Derive the phase from `exec.scenario` stage timing
instead, or break the spike and the recovery window into two separate `scenarios`, each
...
Scenarios let you run several distinct user behaviors side by side, each with its own
threshold. Every `exec` function represents one independent user flow.
...
Reach for the official `grafana/setup-k6-action` rather than a manual apt/gpg install
sequence — it's fewer moving parts, pins an exact version, and can optionally set up
Chrome for `k6/browser` runs.
...
Because k6 exits non-zero the moment any threshold is breached, a broken budget fails
the job automatically — no extra scripting needed. One thing to watch for: **exit code
97** is new in v2 and marks a non-threshold, cloud-side abort — handle that case
explicitly if the pipeline runs against Grafana Cloud k6.

## Lighthouse CI Config

Remember that INP lives only in field data — a Lighthouse lab audit has no way to
measure it. Gate `total-blocking-time` as the stand-in instead, and pull actual INP
numbers from CrUX/RUM (see the "INP only exists in the field" section of SKILL.md).
...
To track trends over time, stand up a self-hosted LHCI server
(`lhci server --storage.storageMethod=sql`) — it makes gradual degradation visible and
lets you line up performance shifts against specific commits.

## k6 Browser Module — Core Web Vitals Under Load

`k6/browser` (built on top of Playwright) is a stable module in both k6 1.x and 2.x.
Reach for it when you need Core Web Vitals numbers while traffic is running, or when you
want to measure INP under a *scripted* interaction (field INP still needs CrUX/RUM to
measure for real). Turn on Chrome in `setup-k6-action` with `browser: true`.
...
Call `page.evaluate` with a `PerformanceObserver` to pull LCP and CLS values in a lab
test.
...
LCP and CLS both report cleanly during a page-load audit like this one. Approximating
INP requires actually driving an interaction (a click or a keystroke) and watching
`event`/`first-input` entries — but that result is still a scripted lab approximation,
not the field INP real users would experience.
