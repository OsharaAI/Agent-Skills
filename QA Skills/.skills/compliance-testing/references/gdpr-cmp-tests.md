...
Runnable Playwright patterns for consent-flow compliance. The reasoning and "what to test" guidance live in `SKILL.md`; this file holds the implementations.
...
GPC is a browser/extension signal communicating a universal opt-out. CCPA/CPRA and most active US state laws require it to be honored. Sites that ignore it are non-compliant in those jurisdictions.

Assert two things: the browser exposes `navigator.globalPrivacyControl === true` (the standard client-side signal — this is the genuine global, not a made-up one), and no marketing cookies are present. There is **no** standard `window.__cmp.gpcStatus` global — TCF v1's `__cmp` is legacy, and TCF v2 uses `__tcfapi` instead (see below). Any CMP-specific opt-out flag you might read is a placeholder that needs replacing with your CMP's actual API.
...
Every IAB TCF-certified CMP (the type required for serving EU/UK ads) exposes `window.__tcfapi`. Read consent state directly through it rather than guessing at CMP-private globals. This is the certified-CMP-agnostic way to confirm a given purpose or vendor has consent.
...
Required since March 2024 for sites serving Google ads in the EEA/UK. Verify the default state is "denied" and that update signals fire correctly after consent choices.
