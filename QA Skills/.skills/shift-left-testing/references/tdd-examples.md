...
Runnable code that supports the dev/QA pairing and TDD facilitation sections of `SKILL.md`. The decision prose, diagrams, and decision guides all live there instead of here.

## Pairing on Test-First Design — Example Output

Here's what a QA + developer pairing session actually produces: agreed test signatures spanning unit, integration, and E2E levels, all written before implementation starts.
...
A complete TDD walk-through: the first failing test, followed by the smallest amount of passing code, followed by a refactor that leaves behavior untouched.
...
Every bug fix begins with a failing test that reproduces the problem; the fix then turns that test green.
