...
Two full `.agents/qa-project-context.md` samples covering different kinds of products.
Borrow the structure, not the version numbers — always check your own framework versions
against `package.json` rather than copying these.
...
A shorter example demonstrating that the same template adapts to a very different kind of product.
...
For a monorepo (identifiable by `turbo.json`, `pnpm-workspace.yaml`, or `nx.json`), give
each frontend app its own entry and roll the shared API into a single one:
...
Shard E2E per app instead of across the whole monorepo — each app's suite should run and
shard on its own, so a change under `apps/admin` never triggers `apps/storefront`'s E2E run.
In the CI/CD section, spell out which path filters gate which app's suite (e.g.
`apps/admin/**` → admin E2E).
