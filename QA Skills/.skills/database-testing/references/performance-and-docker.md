# Query Performance & Docker Test Database — Code Reference

Working examples for EXPLAIN ANALYZE checks, index validation, and Testcontainers/Docker-based test databases. Reasoning, plus the ORM- and MongoDB-specific notes, live in `SKILL.md`.
...
it('should have indexes on frequently queried columns', async () => {
  // Query pg_indexes directly rather than trusting the schema file --
  // this catches an index that was declared but never actually created.
...
## Testcontainers-Based Test Database
...
A performance check that's structurally incapable of failing is the single most common false-positive trap in database testing — it produces confidence without actually verifying anything. Before you trust it, prove it fails when the index is genuinely missing: drop the index and confirm the test goes red, then restore it and confirm the test goes green again.
