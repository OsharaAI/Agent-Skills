...
Working probe code for synthetic monitoring. The decision table for probe design, the non-destructive patterns, and the test-account requirements are documented in `SKILL.md`.
...
Confirms the database and cache connections are healthy without mutating any data — the `/health` endpoint reports each dependency's status, and this probe asserts that both come back `connected`.
...
A proper search probe types a query, submits it, and checks the actual results returned — not just the response status. A status-only search probe will happily pass even when search comes back empty.
...
Probes should switch environments without any code changes — `getProbeConfig()` reads `PROBE_ENVIRONMENT` and returns the matching baseUrl, per-environment credentials, and thresholds (staging's thresholds are set looser than production's).
