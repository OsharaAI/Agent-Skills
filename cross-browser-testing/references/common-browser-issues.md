...
The concrete divergences that show up during cross-browser testing, along with how to catch and address them. `SKILL.md` covers *which* of these actually matter right now; this file is the per-engine tests and CSS workarounds themselves. Every test here checks what the user experiences, not the underlying CSS property.

## Partitioned Cookies / CHIPS (the live 2026 divergence)

Third-party cookies inside an embedded context are handled differently by each engine: Chrome's CHIPS ties a `Partitioned` cookie to the top-level site, Safari's ITP blocks most third-party state outright, and Firefox's State Partitioning isolates it per site. Confirming "the browser supports cookies" isn't sufficient — test the cookie behavior *from inside an iframe*, per engine, and check that the embedded widget still works for the person using it.
...
Flexbox `gap` support is now universal (the old Safari <14.1 bug is long gone). What's still worth keeping is the habit of asserting the *resulting layout* — that cards actually sit side by side — rather than checking a CSS property directly:
