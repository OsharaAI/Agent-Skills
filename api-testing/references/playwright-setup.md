...
`APIRequestContext` runs API tests without launching a browser, and it can still share cookie/storage state with browser contexts when you need that. The `Exploratory vs Automated` tooling comparison and the surrounding reasoning live in `SKILL.md`; this file has the runnable config, the standalone tests, a combined browser+API example, and the authenticated fixture.
