...
Configuration and sample specs for Detox (React Native gray-box testing) and Maestro (declarative cross-platform YAML). The prose behind why you'd pick one lives in `SKILL.md`.
...
> Supported RN range is 0.77–0.84, New Architecture included. Default to `by.id`/`by.text` matchers; reach for `by.type()` only to loosen an overly strict class-based assertion.
