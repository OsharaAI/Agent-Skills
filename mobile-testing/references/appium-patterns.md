...
Install steps, capability objects, element-location code, and gesture snippets for Appium 3.x native/hybrid testing. Framework-choice reasoning and selector priority ordering live in `SKILL.md`.
...
Some shell commands, selector strategies, and device APIs are only available on one OS. Check `platformName` before issuing them — otherwise the command just silently fails on the other platform. A concrete example: the airplane-mode `cmd connectivity` subcommand exists only on newer Android builds and has no iOS equivalent at all.
...
For the complete offline-testing flow built around this guard, see `mobile-patterns.md`.
</content>
