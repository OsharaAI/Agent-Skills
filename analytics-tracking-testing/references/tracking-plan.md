...
The tracking plan is the single source of truth: for each event, it defines the required params and their types. Keep it versioned and import it into both the application and the test suite. Tests check captured events against it rather than hardcoding expected values inline.
...
## YAML form (often more approachable for non-engineers)
...
GA4 splits params into `ep.` (string) and `epn.` (number) prefixes. The validator checks either prefix for presence and correct type, and returns an array of violations — an empty array means the event satisfies the contract.
...
An external plan file gives you a single place to update when a param is renamed, makes the contract reviewable in pull requests, and lets the same schema drive the CI regression gate (see `ci-gating.md`). Hardcoded inline values break the moment anything changes, and they say nothing about params you forgot to list.
